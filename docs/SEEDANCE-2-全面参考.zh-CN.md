...
| `resolution` | 可选：`480p` / `720p` |
| `watermark` | 是否水印 |
| `return_last_frame` | 可选：是否返回尾帧 |
| `safety_identifier` | 可选：安全标识（本仓库对用户有默认派生逻辑） |
| `tools` | 可选：工具列表，例如联网搜索 |

### 3.3 任务状态（终端态）

本仓库将以下状态视为**已结束**，可停止轮询：`succeeded`、`failed`、`cancelled`（`isSeedanceTerminalStatus`）。
...
- **单价**：按「百万 token 人民币」估算，并区分 **是否含参考视频输入**（含视频输入时单价不同）。  
- **输出像素**：按 `resolution` + 固定比例（不含 `adaptive`）映射宽高，再结合 **时长 × 宽 × 高 × 24 / 1024** 估算 token（与常见视频 token 估算思路一致）。

**重要**：表格内数字为**项目内配置**，便于产品演示；**商务与对账务必以官方「模型价格」页与账单为准**。

通过 **Ark Docs MCP** 的 `ark_get_model` 拉取的**列表价（单位：元 / 千 tokens）**与上述「元/百万 tokens」可换算对照（×1000）：

| 模型 ID | `NV2VCompletion`（文档字段名，偏「非 V2V」类输出计价） | `V2VCompletion`（含视频参考等场景常见计价） |
|---------|----------------------------------------------------------|-----------------------------------------------|
| `doubao-seedance-2-0-260128` | 0.046 元/千 tokens → **46 元/百万 tokens** | 0.028 元/千 tokens → **28 元/百万 tokens** |
| `doubao-seedance-2-0-fast-260128` | 0.037 元/千 tokens → **37 元/百万 tokens** | 0.022 元/千 tokens → **22 元/百万 tokens** |
...
- **`resolution`**：`480p` / `720p`；**`1080p` 对 Seedance 2.0 & 2.0 fast 不支持**（文档表格标注）。默认 **720p**。  
...
| 画幅 / 分辨率 | `ratio`、`resolution` | 可选含 `adaptive`；`480p`/`720p`（未提供 1080p，与 2.0 不支持一致） |
...
| 费用感知 | 价目与 usage | 预估（无视频参考时）+ 任务完成后按 `usageTokens` 估算；单价与 MCP `ark_get_model` 列表价一致量级 |
...
| **任务状态 `expired`** | 若接口新增/返回 **`expired`**，`isSeedanceTerminalStatus` / 前台 `isTerminal` 当前**未包含**，可能**一直轮询**；应以 `task.raw` 或后续代码修复为准。 |
| **`usage.tool_usage.web_search`** | 文档可查搜索次数；`SeedanceTaskSummary` **未解析展示**，需看「原始返回」或扩展 `normalizeTaskSummary`。 |
| **`human_face_mode`** | 文档称查询结果可返回；**未解析展示**，同上。 |
...
- 创作台已覆盖 **2.0 主线**：多模态参考、首尾帧模式、联网搜索（纯文）、尾帧衔接、水印、音频开关、历史与费用提示。  
- 若要做成「运维 / 成本可视化」级别：建议补充 **`tool_usage.web_search`**、**`human_face_mode`** 解析，并确认 **`expired`** 等终态与轮询逻辑。  
